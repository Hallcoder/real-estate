...
function ApartmentSelection() {
    ...
    useEffect(()=>{
       client.fetch(`*[_type == 'apartment' && _id == '${params.id}']`).then(data => {
        setProperty(data[0]);
        console.log(data);
        let block = data[0].blocks.find(block => block.blockNumber == params.blockNumber);
        console.log(block)
        setLoading(false);
       })
    },[]);
    ...
    return !loading ? (
        <div
          className="sm:grid flex flex-wrap grid-cols-3 gap-6 mt-20 mx-8 rounded-md"
        >
