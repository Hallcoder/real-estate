...
const PropertyCard: React.FC<Props> = ({
  ...
  return (
    <div className={`flex flex-col m-2 ${width ? 'w-auto':'sm:w-2/3'} border-4 ${className && "slider-item"} rounded-md min-h-auto h-[70vh] cursor-pointer  shadow-md`} onClick={(e) => handeClick(e)}>
      <div className="relative">
        <img src={image} alt="apartment view" className="shadow-md h-52 w-full" />
        {status && <button className="bg-[#333333] p-3 w-7/12 text-white top-[87%] left-[21%] rounded-md absolute">
      ...
      <div className="flex flex-col mt-10 justify-around items-center">
        ...
        <button className="bg-white border-[#333333] border hover:text-white hover:bg-[#333333] p-4 text-xl rounded-full text-[#333333] font-semibold"><MdOutlineStarBorderPurple500 /></button>
          {amenities && <div className="flex items-center justify-around w-4/12">
            <TbBed />{amenities.bedrooms}
            <button >+</button>
            <TbBath />{amenities.baths}
          </div>}
        </div>
      </div>
    </div>
